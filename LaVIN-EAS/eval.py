...
from eas.eval_model import ModelArgs, Transformer
from eas.tokenizer import Tokenizer
from eas.generator import LaVIN_Generator
from eas.mm_adapter import set_MMAdapter, set_Clip_Adapter
from eas.mm_adapter_rep import set_MMAdapter as set_rep_MMAdapter
from eas.mm_adapter_rep import set_Clip_Adapter as set_rep_Clip_Adapter 
...
def load(
    ...
    if max_batch_size > 1:
        ...
        set_rep_MMAdapter(model, adapter_type, dim=adapter_dim, s=adapter_scale,t=temperature)
        set_rep_Clip_Adapter(model.backbone.visual, visual_adapter_type, dim=adapter_dim, s=adapter_scale,t=temperature)
    ...
    for key in adapter_checkpoint['model']:
        ...
        if 'score' in key:
            print(key, adapter_checkpoint['model'][key])

    model.load_state_dict(state_dict, strict=False)
    model.to(torch.device('cuda'))

    for layer in model.layers:
        layer.attention.wq_bak = layer.attention.wq.weight.data
        layer.attention.wk_bak = layer.attention.wk.weight.data
        layer.attention.wv_bak = layer.attention.wv.weight.data
        layer.feed_forward.w1_bak = layer.feed_forward.w1.weight.data
        layer.feed_forward.w3_bak = layer.feed_forward.w3.weight.data
...
def main(
    ...
    temperature,use_vicuna,bits=bits,cpu_load=cpu_load,skip_list=skip_list)
    ...
    pattern1 = re.compile(r'The answer is ([A-Z]).')
    pattern2 = re.compile(r'([A-E]).')
    ...
    for i in range(total_items//max_batch_size+1):
        ...
        for layer in generator.model.layers:
            layer.attention.wq.weight.data = layer.attention.wq_bak 
            layer.attention.wk.weight.data = layer.attention.wk_bak 
            layer.attention.wv.weight.data = layer.attention.wv_bak
            layer.feed_forward.w1.weight.data = layer.feed_forward.w1_bak
            layer.feed_forward.w3.weight.data = layer.feed_forward.w3_bak
            layer.attention.wq.bias.data = torch.zeros_like(layer.attention.wq.bias.data)
            layer.attention.wk.bias.data = torch.zeros_like(layer.attention.wk.bias.data)
            layer.attention.wv.bias.data = torch.zeros_like(layer.attention.wv.bias.data)
        ...
        for result in results:
            pred1 = pattern1.findall(result)
            pred2 = pattern2.findall(result)
    ...
    print(f"Forward in {time.time() - start_time:.2f} seconds")
