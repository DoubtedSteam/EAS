...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (examples, labels, example_mask,images,indicators) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if epoch >= nas_epoch:
            ...
            if nas_count >= nas_turn and epoch >= warmup_epoch:
                nas_count = 0
                ...
                val_loader.sampler.set_epoch((epoch * len(data_loader) + data_iter_step) // nas_turn)
                ...
                torch.distributed.all_reduce(model.module.backbone.visual.scores.data)
                model.module.backbone.visual.scores.data = model.module.backbone.visual.scores.data / world_size
