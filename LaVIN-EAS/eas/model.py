...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.register_buffer('scores', torch.zeros(params.n_layers))
    ...
    def insert_image_embeds(self, examples, labels, image_embeds, prefix_img, prefix_nonimg, img_indicators):
        ...
        for i, (example, label) in enumerate(zip(examples, labels)):
            if img_indicators[i]>0.:
                ...
                new_label = torch.cat([label[:1], torch.zeros(prefix_img.shape[0] + image_embeds.shape[1]).to(examples.device).type_as(labels), label[1:]])
