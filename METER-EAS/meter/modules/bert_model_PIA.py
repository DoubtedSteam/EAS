...
class PIAdapter_Attn(nn.Module):
    ...
    def forward(self, x_0, x_1):
        x_0 = x_0.transpose(1, 2) # B x C x N
        x_1 = x_1.transpose(1, 2) 

        x_1 = self.conv_A(x_1).mean(dim=-1, keepdim=True)
        x = self.dropout(self.conv_C(x_0) + x_1) * 0.5
        
        x = self.conv_B(x) * self.scale
...
class PIAdapter_FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        x_= self.dropout(self.conv_C(x))
        
        x = self.conv_B(x_) * self.scale + x
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.adapter = PIAdapter_FFN(hidden_dim=96)
...
class BertCrossLayer(nn.Module):
    def __init__(self, config):
        ...
        self.adapter_self_attn = PIAdapter_Attn(hidden_dim=96*3//2)
        self.adapter_cross_attn = PIAdapter_Attn(hidden_dim=96*3//2)
        
        self.skip_self_attn = False
        self.skip_cross_attn = False
    ...
    def forward(
        ...
        if not self.skip_self_attn:
            ...
            attention_output = self.attention.output.LayerNorm(hidden_states + self.adapter_self_attn(hidden_states, hidden_states))
        ...
        if not self.skip_cross_attn:
            ...
            attention_output = self.crossattention.output.LayerNorm(attention_output + self.adapter_cross_attn(hidden_states, encoder_hidden_states))
